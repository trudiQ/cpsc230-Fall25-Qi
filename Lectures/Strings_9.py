...
if my_string[0].isupper(): # access the first character in my_string
    print(f"{my_string}'s first character is capatlized.") # using f-string
    #print(my_string, "'s first character is capatlized.")
else:
    print(f"{my_string}'s first character is not capatlized.") # using f-string
...
# c.
my_tokens = my_string.split() # split the string into a list of substrings (tokens)
token_count = len(my_tokens) # len() function returns the number of items (tokens) in the list
print(f"There are {token_count} number of words.")
