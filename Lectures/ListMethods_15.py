...
print(tasks)
# b)
tasks.insert(1, "Buy gcroceries")
print(tasks)
# c)
last_task = tasks.pop(-1) # or, pop()
print(last_task)
# d)
...
# e) 
temp_ascend = sorted(temperatures) # reverse = False (default argument)
...
# c)
# soluton 1 - RECOMMENDED:
index_list = [] # empty list
for i in range(len(temperatures)):
    temp = temperatures[i]
    if temp > 74:
        index_list.append(i) # adding index of each value > 74
print("temp>74 index list: ", index_list)

# solution 2 - CORRECT BUT NOT ALIGNED WITH THE QUESTION (not using range())
index_list = [] # empty list
for i in temperatures:
    if i > 74:
        i_index = temperatures.index(i) # i is temperature
